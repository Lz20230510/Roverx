...
void GUI_DeleteDot(u16 x,u16 y)
{
	BEGIN_CMD();
	TX_8(0x58);
	TX_16(x);
...
void GraphSetViewport(u16 screen_id,u16 control_id,short int x_offset,u16 x_mul,short int y_offset,u16 y_mul)
{
	BEGIN_CMD();
	TX_8(0xB1);
	TX_8(0x34);
...
void TriggerRowDate(u16 screen_id,u16 control_id,u16 MsgNum)
{
	BEGIN_CMD();
	TX_8(0xB1);
	TX_8(0x50);
...
void RemoveRowDate(u16 screen_id,u16 control_id,u16 MsgNum)
{
	BEGIN_CMD();
	TX_8(0xB1);
	TX_8(0x51);
...
void DeleteAllDate(u16 screen_id,u16 control_id)
{
	BEGIN_CMD();
	TX_8(0xB1);
	TX_8(0x53);
...
void DeleteRowDate(u16 screen_id,u16 control_id,u16 rowNum)
{
	BEGIN_CMD();
	TX_8(0xB1);
	TX_8(0x58);
...
/***************************对某一页面参数进行刷新************************/
//void OnePg_Refresh(u16 screenID ,TextFormat *textFormat,u16 num,u8 type,u8 mode)  
//输入参数:u16 screenID :页面号TextFormat *textFormat:参数数据存储  u16 num：参数数量   u8 type：数据类型
//           u8 mode:      mode=0:文本控件，   mode=1:图标控件
//输出参数：
//返回：
//注意:screenID就是pageID
/*****************************************************************************/
	...
	{	
		case _u8:							//u8类型
		{
			...
			{										
				for(i=0;i<num;i++)
				{
					sprintf((char*)buffer,"%u",textFormat[i].u8Text); //把整数转换为字符串
					TX_16(textFormat[i].controlID);
					TX_16((u16)strlen((const char *)buffer));				
					SendStrings(buffer);
				}		
			}
			else if(mode==1)		//图标控件
			{
				for(i=0;i<num;i++)
				{
					TX_16(textFormat[i].controlID);		//控件号
					TX_16(0x0002);										//数据长度2
					TX_16(textFormat[i].u8Text);			//图标序列号
				}
			}	
			END_CMD();
		}break;
		case _u16:							//u16类型
		{
			...
			for(i=0;i<num;i++)
			{
				sprintf((char*)buffer,"%u",textFormat[i].u16Text); //把整数转换为字符串			
				TX_16(textFormat[i].controlID);
				TX_16((u16)strlen((const char *)buffer));				
				SendStrings(buffer);
			}		
		...
		case _u32:							//u32类型
		{
			...
			for(i=0;i<num;i++)
			{
				sprintf((char*)buffer,"%u",textFormat[i].u32Text); //把整数转换为字符串
				TX_16(textFormat[i].controlID);
				TX_16((u16)strlen((const char *)buffer));				
				SendStrings(buffer);
			}		
		...
		{	
			...
			for(i=0;i<num;i++)
			{
				sprintf((char*)buffer,"%.2f",textFormat[i].floatText); //把整数转换为字符串
				TX_16(textFormat[i].controlID);
				TX_16((u16)strlen((const char *)buffer));				
				SendStrings(buffer);
			}		
		...
		case _double:										//double类型
		{
			...
			for(i=0;i<num;i++)
			{
				sprintf((char*)buffer,"%lf",textFormat[i].doubleText); //把整数转换为字符串
				TX_16(textFormat[i].controlID);
				TX_16((u16)strlen((const char *)buffer));				
				SendStrings(buffer);
			}		
...
/******************************对图标控件进行批量刷新*****************************/
//void MultiIcon_Refresh(u16 screenID,u16 firstCotlID,u16 lastCotlID,u8*iconDate，u8 mode)
//输入参数:u16 screenID:页面ID号   firstCotlID：起始图标控件号		lastCotlID：最后图标控件号	 u8*iconDate：icon数值
//u8 mode： =0：图标类型   =1：文本类型
//输出参数:
//
//返回：
//
...
void MultiIcon_Refresh(u16 screenID,u16 firstCotlID,u16 lastCotlID,u8*iconDate,u8 mode)
{
	...
	switch(mode)			//判断控件类型
	{
		case 0:					//图标类型
		{
			for(i=firstCotlID;i<=lastCotlID;i++) 
			{
				TX_16(i);					//控件号
				TX_16(0x02);			//长度
				TX_16(*iconDate);			//值
		...
		case 1:					//文本类型
		{
			for(i=firstCotlID;i<=lastCotlID;i++) 
			{
				sprintf((char*)buffer,"%X",*iconDate); //把整数转换为字符串
				TX_16(i);
				TX_16((u16)strlen((const char *)buffer));				
				SendStrings(buffer);
				iconDate++;
...
/*****************************循环刷新某一界面****************************/
//void Cycle_SetScreen(u16 screenID,u8 times)
//输入参数:u16 screenID:页面ID号  u8 times：循环次数
//输出参数:
//返回：
//
/*********************************************************************************/
