...
void lwip_comm_default_ip_set(__lwip_dev *lwipx)
{
	...
	//默认远端IP为:192.168.10.151
	lwipx->remoteip[0]=192;	
	lwipx->remoteip[1]=168;
	lwipx->remoteip[2]=10;
	lwipx->remoteip[3]=151;
...
void lwip_dhcp_task(void *pdata)
{
	...
	{ 
		printf("正在获取地址...\r\n");
		if(lwip_netif.dhcp->state == DHCP_BOUND)
		{
			printf("获取到地址...\r\n");
		}
		else
		...
		if(ip!=0)   					//当正确读取到IP地址的时候
		{
			lwipdev.dhcpstatus=2;	//DHCP成功
 			printf("[DHCP] 网卡en的MAC地址为:................%d.%d.%d.%d.%d.%d\r\n",lwipdev.mac[0],lwipdev.mac[1],lwipdev.mac[2],lwipdev.mac[3],lwipdev.mac[4],lwipdev.mac[5]);
			//解析出通过DHCP获取到的IP地址
			lwipdev.ip[3]=(uint8_t)(ip>>24); 
			lwipdev.ip[2]=(uint8_t)(ip>>16);
			lwipdev.ip[1]=(uint8_t)(ip>>8);
			lwipdev.ip[0]=(uint8_t)(ip);
			printf("[DHCP] 通过DHCP获取到IP地址..............%d.%d.%d.%d\r\n",lwipdev.ip[0],lwipdev.ip[1],lwipdev.ip[2],lwipdev.ip[3]);
			//解析通过DHCP获取到的子网掩码地址
			lwipdev.netmask[3]=(uint8_t)(netmask>>24);
			lwipdev.netmask[2]=(uint8_t)(netmask>>16);
			lwipdev.netmask[1]=(uint8_t)(netmask>>8);
			lwipdev.netmask[0]=(uint8_t)(netmask);
			printf("[DHCP] 通过DHCP获取到子网掩码............%d.%d.%d.%d\r\n",lwipdev.netmask[0],lwipdev.netmask[1],lwipdev.netmask[2],lwipdev.netmask[3]);
			//解析出通过DHCP获取到的默认网关
			lwipdev.gateway[3]=(uint8_t)(gw>>24);
			lwipdev.gateway[2]=(uint8_t)(gw>>16);
			lwipdev.gateway[1]=(uint8_t)(gw>>8);
			lwipdev.gateway[0]=(uint8_t)(gw);
			printf("[DHCP] 通过DHCP获取到的默认网关..........%d.%d.%d.%d\r\n",lwipdev.gateway[0],lwipdev.gateway[1],lwipdev.gateway[2],lwipdev.gateway[3]);
			//当前设定的默认远程端地址
			printf("[DHCP] 当前设置的默认远程端地址..........%d.%d.%d.%d\r\n",lwipdev.remoteip[0],lwipdev.remoteip[1],lwipdev.remoteip[2],lwipdev.remoteip[3]);
			//解析出通过DHCP获取到远程端地址
			printf("[DHCP] 通过DHCP获取到的远程端端口号......%d\r\n",UDP_DEMO_PORT);//客户端端口号
...
//	while(udp_demo_init()==pdFAIL)							//当完成了DHCP任务之后，再开启UDP任务。初始化udp_demo(创建udp_demo线程),实验验证开启UDP之前，不必先完成DHCP的IP分配。
//	{
//		LCD_ShowString(30,190,200,20,16,"UDP failed!!");	//udp创建失败
//		delay_ms(500);
//		LCD_Fill(30,190,230,170,WHITE);
//		delay_ms(500);
//	}		
//	LCD_ShowString(30,190,200,20,16,"UDP Success!");		//udp创建成功
